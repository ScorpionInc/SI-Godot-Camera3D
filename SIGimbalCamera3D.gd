...
class_name SIGimbalCamera3D extends Camera3D
...
const DEFAULT_ROTATION_DEGREES_MIN:float = rad_to_deg(DEFAULT_ROTATION_MIN)
const DEFAULT_ROTATION_DEGREES_MAX:float = rad_to_deg(DEFAULT_ROTATION_MAX)
...
@export var debug_base_color:Color = Color.CRIMSON
@export var debug_arm_color:Color = Color.AQUA
...
var rotation_base:Node3D = null
var rotation_arm:Node3D = null

#Camera Distance(in/out)
var distance_direction:float = 0.0#What direction in/out did we last get an input event for?
@export var distance:float = DEFAULT_CAMERA_DISTANCE : set = distance_set# Current distance between camera and origin
@export var distance_increase_action_name:String = DEFAULT_DISTANCE_INCREASE_ACTION_NAME# Name of the input action group used in increase the target distance value
@export var distance_decrease_action_name:String = DEFAULT_DISTANCE_DECREASE_ACTION_NAME# Name of the input action group used in decrease the target distance value
@export var distance_target:float = DEFAULT_CAMERA_DISTANCE# Current distance from origin the camera should be moving to
@export var distance_min:float = DEFAULT_CAMERA_DISTANCE_MIN#TODO
@export var distance_max:float = DEFAULT_CAMERA_DISTANCE_MAX#TODO
@export var distance_acceleration:float = DEFAULT_CAMERA_DISTANCE_ACCELERATION#How fast does the target distance change per input frame?
@export var distance_target_acceleration:float = self.distance_acceleration#How fast should the actual distance change towards the target distance?(Smoothing)
@export var distance_velocity:float = 0.0#How fast are we currently moving the target distance?
@export var distance_dampening:float = 0.1#How much should we slow down the distance velocity every frame?
#Left/Right
@export var yaw:float = DEFAULT_YAW : set = yaw_set# Current yaw in radians
@export var yaw_degrees:float = rad_to_deg(yaw) : set = yaw_degrees_set# Current yaw in degrees
@export var yaw_increase_action_name:String = DEFAULT_YAW_INCREASE_ACTION_NAME# Name of the input action group used in increase the target yaw value
@export var yaw_decrease_action_name:String = DEFAULT_YAW_DECREASE_ACTION_NAME# Name of the input action group used in decrease the target yaw value
@export var yaw_target:float = DEFAULT_YAW : set = yaw_target_set# The current yaw value the camera should be rotating to in radians
@export var yaw_target_degrees:float = rad_to_deg(yaw_target) : set = yaw_target_degrees_set# The current yaw value the camera should be rotating to in degrees
@export var yaw_min:float = DEFAULT_ROTATION_MIN : set = yaw_min_set# Limit the minimum yaw value of the current camera in radians. Always positive.
@export var yaw_max:float = DEFAULT_ROTATION_MAX : set = yaw_max_set# Limit the maximum yaw value of the current camera in radians. Always positive.
@export var yaw_degrees_min:float = DEFAULT_ROTATION_DEGREES_MIN : set = yaw_degrees_min_set# Limit the minimum yaw value of the current camera in degrees. Always positive.
@export var yaw_degrees_max:float = DEFAULT_ROTATION_DEGREES_MAX : set = yaw_degrees_max_set# Limit the maximum yaw value of the current camera in degrees. Always positive.
#Forward/Backward
@export var pitch:float = DEFAULT_YAW : set = pitch_set# Current pitch in radians
@export var pitch_degrees:float = rad_to_deg(yaw) : set = pitch_degrees_set# Current pitch in degrees
@export var pitch_increase_action_name:String = DEFAULT_PITCH_INCREASE_ACTION_NAME# Name of the input action group used in increase the target pitch value
@export var pitch_decrease_action_name:String = DEFAULT_PITCH_DECREASE_ACTION_NAME# Name of the input action group used in decrease the target pitch value
@export var pitch_target:float = DEFAULT_YAW# The current pitch value the camera should be rotating to in radians
@export var pitch_target_degrees:float = rad_to_deg(yaw_target)# The current pitch value the camera should be rotating to in degrees
@export var pitch_min:float = DEFAULT_ROTATION_MIN : set = pitch_min_set# Limit the minimum pitch value of the current camera in radians. Always positive.
@export var pitch_max:float = DEFAULT_ROTATION_MAX : set = pitch_max_set# Limit the maximum pitch value of the current camera in radians. Always positive.
@export var pitch_degrees_min:float = DEFAULT_ROTATION_DEGREES_MIN : set = pitch_degrees_min_set# Limit the minimum pitch value of the current camera in degrees. Always positive.
@export var pitch_degrees_max:float = DEFAULT_ROTATION_DEGREES_MAX : set = pitch_degrees_max_set# Limit the maximum pitch value of the current camera in degrees. Always positive.
#Roll
@export var roll:float = DEFAULT_YAW : set = roll_set# Current roll in radians
@export var roll_degrees:float = rad_to_deg(yaw) : set = roll_degrees_set# Current roll in degrees
@export var roll_increase_action_name:String = DEFAULT_ROLL_INCREASE_ACTION_NAME# Name of the input action group used in increase the target roll value
@export var roll_decrease_action_name:String = DEFAULT_ROLL_DECREASE_ACTION_NAME# Name of the input action group used in decrease the target roll value
@export var roll_target:float = DEFAULT_YAW# The current roll value the camera should be rotating to in radians
@export var roll_target_degrees:float = rad_to_deg(yaw_target)# The current roll value the camera should be rotating to in degrees
@export var roll_min:float = DEFAULT_ROTATION_MIN : set = roll_min_set# Limit the minimum roll value of the current camera in radians. Always positive.
@export var roll_max:float = DEFAULT_ROTATION_MAX : set = roll_max_set# Limit the maximum roll value of the current camera in radians. Always positive.
@export var roll_degrees_min:float = DEFAULT_ROTATION_DEGREES_MIN : set = roll_degrees_min_set# Limit the minimum roll value of the current camera in degrees. Always positive.
@export var roll_degrees_max:float = DEFAULT_ROTATION_DEGREES_MAX : set = roll_degrees_max_set# Limit the maximum roll value of the current camera in degrees. Always positive.

#Collision
@export_flags_3d_physics var physics_mask:int = DEFAULT_PHYSICS_MASK
@export var collide_camera:bool = false#TODO Camera collides with physics during movements/rotations.
...
@export var rotation_velocity_vector:Vector3 = Vector3.ZERO#TODO How fast are we currently rotating the target each frame per axis?
@export var rotation_target_acceleration_vector:Vector3 = Vector3(self.TWO_PIE, self.TWO_PIE, self.TWO_PIE)#Disabled How fast towards the target do we accelerate?
@export var rotation_acceleration_vector:Vector3 = Vector3(PI / 16, PI / 16, PI / 16)#TODO How fast per frame do we change our velocity?
@export var rotation_dampening_vector:Vector3 = Vector3(PI / 32, PI / 32, PI / 32)#TODO How much per frame should we slow down our velocity without input.

#Mouse
@export var use_scroll_wheel:bool = DEFAULT_SCROLL_WHEEL_USAGE#TODO
@export var use_mouse_movement:bool = false#TODO
@export_range(-1.0, 3.0) var mouse_sensitivity:float = 1.0#Scalar
@export var capture_mouse:bool = false#TODO
@export var require_left_drag:bool = false#TODO
@export var require_middle_drag:bool = false#TODO
@export var require_right_drag:bool = false#TODO
...
#Movement
#TODO
@export var allow_camera_movement:bool = false
@export var move_parent:bool = false
...
@export var movement_forward_action_name:String = "camera_move_forward"#TODO
@export var movement_backward_action_name:String = "camera_move_backward"#TODO
@export var movement_left_action_name:String = "camera_move_left"#TODO
@export var movement_right_action_name:String = "camera_move_right"#TODO
@export var movement_up_action_name:String = "camera_move_up"#TODO
@export var movement_down_action_name:String = "camera_move_down"#TODO
@export var movement_velocity_vector:Vector3 = Vector3.ZERO#TODO How fast are we currently moving the target position each frame per axis?
@export var movement_acceleration_vector:Vector3 = Vector3(3.0, 3.0, 3.0)#TODO How fast per frame do we change our velocity?
@export var movement_target_acceleration_vector:Vector3 = self.movement_acceleration_vector#Disabled How fast towards the target do we accelerate?
@export var movement_dampening_vector:Vector3 = Vector3(0.1, 0.1, 0.1)#TODO How much per frame should we slow down our velocity without input.
@export var movement_maximum_velocity:Vector3 = Vector3.INF#Disabled
...
func setupCameraRotationNodes():
	...
	if(not hasArmParent()):
		self.rotation_arm = MeshInstance3D.new()#Later on may make visible for debugging
		if(not self.debug_mesh == null):
			self.rotation_arm.mesh = self.debug_mesh.duplicate(true)
			if(self.rotation_arm.mesh.material == null):
				self.rotation_arm.mesh.material = StandardMaterial3D.new()
			if(self.rotation_arm.mesh.material is StandardMaterial3D):
		...
		rotation_arm.tree_entered.connect(self.onArmAdded)
		self.rotation_arm.position = self.position#Move arm node to camera
	...
	if(not hasBaseSpatial()):
		self.rotation_base = MeshInstance3D.new()#Later on may make visible for debugging
		if(not self.debug_mesh == null):
			self.rotation_base.mesh = self.debug_mesh.duplicate(true)
			if(self.rotation_base.mesh.material == null):
				self.rotation_base.mesh.material = StandardMaterial3D.new()
			if(self.rotation_base.mesh.material is StandardMaterial3D):
		...
		rotation_base.tree_entered.connect(self.onBaseAdded)
		self.rotation_base.position = rotation_arm.position#Move base node to the arm
		self.rotation_arm.position = Vector3.ZERO#Reset the arm's translation to the base
	...
	self.position = Vector3(0.0, 0.0, self.distance)#Initialize distance
...
############################
func _ready():
	self.get_tree().get_root().size_changed.connect(self.onWindowResize)
...
func _input(_event):
	...
	if(_event is InputEventMouseButton):
		if(_event.pressed):
			...
			elif(_event.button_index == MOUSE_BUTTON_MIDDLE):
...
func _physics_process(_delta):
	...
	if(self.allow_camera_movement):
		if(self.move_parent):
			if(self.hasArmParent() and self.hasBaseSpatial()):
				self.rotation_base.get_parent_spatial().position += self.movement_velocity_vector.rotated(Vector3.UP, self.yaw)
			else:
				self.get_parent_node_3d().position += self.movement_velocity_vector.rotated(Vector3.UP, self.yaw)
		else:
			if(self.hasArmParent() and self.hasBaseSpatial()):
				self.rotation_base.position += self.movement_velocity_vector.rotated(Vector3.UP, self.yaw)
			else:
				self.position += self.movement_velocity_vector.rotated(Vector3.UP, self.yaw)
	...
	if(not self.hasArmParent() or not self.hasBaseSpatial() or not self.collide_camera):
		return
	#Cast shape from base to camera, not the other way around. This will collide with the closest/nearest object first.
	...
	shape_cast.transform.origin = self.rotation_base.global_position
	shape_cast.transform = shape_cast.transform.looking_at(self.global_position, Vector3.UP, false)
	var shape:BoxShape3D = BoxShape3D.new()
	shape.size = Vector3(self.near, self.near, self.distance)
	shape_cast.shape = shape
	shape_cast.exclude = [self, self.rotation_arm, self.rotation_base]
	shape_cast.collision_mask = self.DEFAULT_PHYSICS_MASK
	var collisions = space_state.collide_shape(shape_cast, 32)
	if(not collisions.is_empty()):
		print(str(collisions))
	var next_collision_distance:float = 0.0
	for collision_point:Vector3 in collisions:
		next_collision_distance = self.rotation_base.global_position.distance_to(collision_point)
		if(next_collision_distance < self.distance):
			self.distance = next_collision_distance
	...
	#var collision = space_state.intersect_ray(PhysicsRayQueryParameters3D.create(self.rotation_base.global_position, self.global_position, self.physics_mask, [self, self.rotation_arm, self.rotation_base]))
	#if(not collision.is_empty()):
		#self.distance = (self.rotation_base.global_position.distance_to(collision.position))
...
func yaw_set( new_value:float ):
	...
	if(yaw != new_value):
		yaw = new_value
		yaw_degrees = rad_to_deg(new_value)
	#Apply
	self._updateYaw()
func yaw_degrees_set( new_value:float ):
	...
	#Update
	if(yaw_degrees != new_value):
		yaw = deg_to_rad(new_value)
...
func yaw_target_set( new_value:float ):
	...
	new_value = self.applyMinimumWithPossibleMaximum(new_value, self.yaw_min, self.yaw_max)
	#Update
	if(yaw_target != new_value):
		yaw_target = new_value
		yaw_target_degrees = rad_to_deg(new_value)
func yaw_target_degrees_set( new_value:float ):
	#Apply limits
	new_value = fmod(new_value, 360.0)#Wraps around using modulo
	new_value = self.applyMinimumWithPossibleMaximum(new_value, self.yaw_degrees_min, self.yaw_degrees_max)
	#Update
	if(yaw_target_degrees != new_value):
		yaw_target = deg_to_rad(new_value)
...
func pitch_set( new_value:float ):
	...
	if(pitch != new_value):
		pitch = new_value
		pitch_degrees = rad_to_deg(new_value)
	#Apply
	self._updatePitch()
func pitch_degrees_set( new_value:float ):
	...
	#Update
	if(pitch_degrees != new_value):
		pitch = deg_to_rad(new_value)
...
func pitch_target_set( new_value:float ):
	...
	new_value = self.applyMinimumWithPossibleMaximum(new_value, self.pitch_min, self.pitch_max)
	#Update
	if(pitch_target != new_value):
		pitch_target = new_value
		pitch_target_degrees = rad_to_deg(new_value)
func pitch_target_degrees_set( new_value:float ):
	#Apply limits
	new_value = fmod(new_value, 360.0)#Wraps around using modulo
	new_value = self.applyMinimumWithPossibleMaximum(new_value, self.pitch_degrees_min, self.pitch_degrees_max)
	#Update
	if(pitch_target_degrees != new_value):
		pitch_target = deg_to_rad(new_value)
...
func roll_set( new_value:float ):
	...
	if(roll != new_value):
		roll = new_value
		roll_degrees = rad_to_deg(new_value)
	#Apply
	self._updateRoll()
func roll_degrees_set( new_value:float ):
	...
	#Update
	if(roll_degrees != new_value):
		roll = deg_to_rad(new_value)
...
func roll_target_set( new_value:float ):
	...
	new_value = self.applyMinimumWithPossibleMaximum(new_value, self.roll_min, self.roll_max)
	#Update
	roll_target_degrees = rad_to_deg(new_value)
...
func roll_target_degrees_set( new_value:float ):
	...
	new_value = self.applyMinimumWithPossibleMaximum(new_value, self.roll_degrees_min, self.roll_degrees_max)
	#Update
	roll_target = deg_to_rad(new_value)
